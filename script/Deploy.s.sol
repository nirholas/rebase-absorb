...
/**
 * @title DeployRebaseAbsorb
 * @notice Deploys RebaseAbsorb deterministically to any chain with a Uniswap v4 `PoolManager`.
 *
 ...
 */
contract DeployRebaseAbsorb is Script {
    ...
    function run() external {
        ...
        bytes memory creationCode = type(RebaseAbsorbHook).creationCode;
        bytes memory constructorArgs = abi.encode(manager, _swapFeeBps, shareName, shareSymbol);
        ...
        console2.log("RebaseAbsorb ", predicted);
        ...
        vm.startBroadcast();
        RebaseAbsorbHook hook = new RebaseAbsorbHook{salt: salt}(manager, _swapFeeBps, shareName, shareSymbol);
        vm.stopBroadcast();
        ...
        require(
            keccak256(bytes(IHookMetadata(address(hook)).hookName())) == keccak256(bytes("RebaseAbsorb")),

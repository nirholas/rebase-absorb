...
contract DeployLocal is Script {
    ...
    function run() external {
        ...
        (address predicted, bytes32 salt) = HookMiner.find(
            CREATE2_DEPLOYER, FLAGS, type(RebaseAbsorbHook).creationCode, abi.encode(IPoolManager(address(manager)), 30, "Rebase Absorb LP", "RA-LP")
        );
        hook = new RebaseAbsorbHook{salt: salt}(IPoolManager(address(manager)), 30, "Rebase Absorb LP", "RA-LP");
